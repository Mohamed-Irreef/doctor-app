...
import { DOCTORS, CATEGORIES } from '../../constants/MockData';
import { useFavoritesStore } from '../../store/favoritesStore';
import { ArrowLeft, Search, Heart, Star, SlidersHorizontal, X } from 'lucide-react-native';
import { useRouter, useLocalSearchParams } from 'expo-router';
import type { Doctor } from '../../types';
import { ListSkeleton } from '../../components/SkeletonLoader';

const SORT_OPTIONS = ['Relevance', 'Rating', 'Experience', 'Fee: Low', 'Fee: High'];
const SPECIALTY_FILTERS = ['All', ...CATEGORIES.slice(0, 6).map(c => c.name)];

function DoctorRow({ item, onPress, onFav, faved }: { item: Doctor; onPress: () => void; onFav: () => void; faved: boolean }) {
  return (
    <TouchableOpacity style={styles.docRow} onPress={onPress} activeOpacity={0.85}>
      ...
      <View style={styles.docInfo}>
        ...
        <View style={styles.docMeta}>
          ...
          <Text style={styles.docRating}>{item.rating} ({item.reviews})</Text>
          <View style={styles.dot} />
          <Text style={styles.docExp}>{item.experience}</Text>
        </View>
        <Text style={styles.docHospital} numberOfLines={1}>{item.hospital ?? 'Multi-specialty Hospital'}</Text>
        <View style={styles.docFooter}>
          <Text style={styles.docFee}>₹{item.fee} <Text style={{ fontSize: 11, fontWeight: '400', color: Colors.textSecondary }}>/ visit</Text></Text>
      ...
      <TouchableOpacity onPress={onFav} style={styles.favBtn} activeOpacity={0.8} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
        <Heart size={18} color={faved ? Colors.error : Colors.textSecondary} fill={faved ? Colors.error : 'none'} />
      </TouchableOpacity>
    </TouchableOpacity>
  );
}
...
export default function SearchDoctorScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    switch (activeSort) {
      ...
      case 'Experience':  return list.sort((a, b) => parseInt(b.experience) - parseInt(a.experience));
      case 'Fee: Low':    return list.sort((a, b) => a.fee - b.fee);
      case 'Fee: High':   return list.sort((a, b) => b.fee - a.fee);
      default:            return list;
    }
  }, [query, activeSpec, activeSort]);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => setShowSort(s => !s)} style={styles.sortBtn} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
      ...
      {showSort && (
        <View style={styles.sortPanel}>
          <Text style={[Typography.body2, { fontWeight: '700', marginBottom: 10 }]}>Sort By</Text>
          {SORT_OPTIONS.map(s => (
            <TouchableOpacity
              key={s}
      ...
      >
        {SPECIALTY_FILTERS.map(spec => (
          <TouchableOpacity
            ...
            activeOpacity={0.75}
          >
            <Text style={[styles.chipText, activeSpec === spec && styles.chipTextActive]}>{spec}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      <View style={styles.resultRow}>
        ...
        {activeSort !== 'Relevance' && (
          <TouchableOpacity onPress={() => setActiveSort('Relevance')} style={styles.clearSort}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <TouchableOpacity onPress={() => { setQuery(''); setActiveSpec('All'); }} style={styles.clearBtn}>
              <Text style={{ color: Colors.surface, fontWeight: '700' }}>Clear Filters</Text>
            </TouchableOpacity>
          </View>
        }
        renderItem={({ item }) => (
...
const styles = StyleSheet.create({
  ...
  sortBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#EFF6FF', alignItems: 'center', justifyContent: 'center' },
  sortPanel: {
    position: 'absolute', top: 64, right: 16, zIndex: 99,
    backgroundColor: Colors.surface, borderRadius: 16, padding: 16,
    borderWidth: 1, borderColor: Colors.border,
    shadowColor: Colors.black, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 12, elevation: 8,
  },
  sortOption: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 10, marginBottom: 4 },
  ...
  resultRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 10 },
  resultCount: { fontSize: 13, color: Colors.textSecondary, fontWeight: '500' },
  clearSort: { backgroundColor: '#EFF6FF', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 10 },
  clearSortText: { fontSize: 12, color: Colors.primary, fontWeight: '600' },
  listContent: { paddingHorizontal: 16, paddingBottom: 40 },
  docRow: {
    flexDirection: 'row', backgroundColor: Colors.surface, borderRadius: 18, marginBottom: 14,
  ...
  docName: { fontSize: 15, fontWeight: '700', color: Colors.text, marginBottom: 2 },
  ...
  docRating: { fontSize: 12, fontWeight: '700', color: '#D97706', marginLeft: 4 },
  ...
  availBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#DCFCE7', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10 },
  availDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#16A34A', marginRight: 4 },
  ...
  clearBtn: { backgroundColor: Colors.primary, paddingHorizontal: 32, paddingVertical: 14, borderRadius: 14 },
});
